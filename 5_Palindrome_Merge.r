...
-------------------------------------
 Writing all output into another File |
-------------------------------------
display_menu <- function() {
  cat("Menu:\n")
  cat("1. Merge\n")
...
merge1 <- function() {
  ...
  cat("Merged DataFrame saved to 'merged_output.csv'\n")
}
...
palindrome <- function() {
  ...
  palindromic_names <- df[unlist(lapply(df$Name, is_palindrome)), c("ID", "Name")]
  write.csv(palindromic_names, "palindromic_names.csv", row.names = FALSE)
  cat("Palindromic names saved to 'palindromic_names.csv'\n")
}
...
max_and_min <- function() {
  ...
  results <- data.frame(RowMax = max_values_rows, RowMin = min_values_rows)
  results <- cbind(results, t(data.frame(ColMax = max_values_columns, ColMin = min_values_columns)))
  write.csv(results, "max_min_output.csv", row.names = FALSE)
  cat("Max and Min values saved to 'max_min_output.csv'\n")
}
...
check_fibonacci_and_factorial <- function(df) {
  results <- data.frame(Number = integer(), Factorial = integer())
  
  for (num in df$numbers) { 
    if (is_fibonacci(num)) {
      results <- rbind(results, data.frame(Number = num, Factorial = factorial(num)))
    }
  }
  
  write.csv(results, "fibonacci_factorial_output.csv", row.names = FALSE)
  cat("Fibonacci factorial values saved to 'fibonacci_factorial_output.csv'\n")
}
